...
(() => {
  ...
  async function edcMostrarEstadoCuentaAreaModal(paciente, areaSel, opts = {}) {
    try {
      ...
      const MODULOS = MODULOS_ALL.filter((m) => {
        ...
        if (areaNombre && mAreaNom && mAreaNom.toLowerCase() === areaNombre.toLowerCase())
          return true;
      ...
      let facturas = facturasRaw.map((f) => {
        ...
        // ✅ NUEVO: paga/impaga (persistido)
        const facturaPagada =
          f.facturaPagada === true ||
          String(f.estado || f.estadoFactura || "").toUpperCase() === "PAGADO"; // compat

        return { mes, nro, monto, detalle, facturaPagada };
      });

      if (!facturas.length) {
        facturas.push({ mes: "", nro: "", monto: 0, detalle: "", facturaPagada: false });
      }
      ...
      const areaColor = (() => {
        ...
        if (nNorm.includes("abordaje integral") || nNorm.includes("discapacidad") || nNorm.includes("a.i"))
          return "#373DB6";
        return "#8BCB86";
      })();
      ...
      <div id="edcModalRoot" style="text-align:left;font-family:'Segoe UI',sans-serif;">
        <div style="margin-bottom:8px; display:flex; align-items:center; gap:10px; flex-wrap:wrap;">
          ...
          <div style="display:flex; align-items:center; gap:8px;">
            <strong>Desde:</strong>
            <input id="edcDesde" type="month" value="${desdeSel}"
  ...
  <tr class="edc-th">
    ...
    <th class="edc-col-apag">MONTO</th>
    <th class="edc-col-obs">DETALLE</th>
    <th style="min-width:70px; text-align:center;">PAGA</th>
  </tr>
</thead>
    ...
    `;
      ...
      await Swal.fire({
        ...
        didOpen: (popup) => {
          ...
          const $desde = popup.querySelector("#edcDesde");
          const $hasta = popup.querySelector("#edcHasta");
      ...
      <tr>
        ...
        <td style="text-align:center;min-width:70px;">
          <input type="checkbox"
            data-idx="${idx}" data-field="facturaPagada"
            class="edc-input-fact"
            ${f.facturaPagada ? "checked" : ""}>
        </td>
      </tr>
    `
              )
          ...
          const handleChange = (e) => {
            ...
            if (t.classList.contains("edc-input-fact")) {
              if (field === "monto") facturas[idx].monto = safeNum(t.value);
              else if (field === "facturaPagada") facturas[idx].facturaPagada = t.checked;
              else facturas[idx][field] = t.value;
              render();
              return;
            }
          };
          ...
          if (selAreaModal) {
            selAreaModal.addEventListener("change", () => {
              ...
              Swal.close();
              edcMostrarEstadoCuentaAreaModal(paciente, nuevaArea, { desde, hasta });
            });
          }
